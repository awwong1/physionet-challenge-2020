...
# rerun the experiment, but have lead_feature_selection per model
...
do
    ...
    do
        python3 main.py configs/scikit_learn/base.json \
            --override "{\
                \"exp_name\": \"PhysioNet2020/Scikit_Learn_RFC_Feature_Selection/${classifier_name}/cv-${i}\", \
                \"cross_validation\": {\"val_offset\": ${i}}, \
                \"lead_pipeline_feature_selection\": {\"name\": \"sklearn.ensemble.RandomForestClassifier\"}, \
                \"lead_classifier\": {\"name\": \"${classifier}\"} \
            }"
    done
done
