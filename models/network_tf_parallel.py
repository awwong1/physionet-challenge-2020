...
def build_network(**params):
    ...
    from tensorflow.keras.layers import Input, Lambda, Concatenate, Flatten
    ...
    for lead in range(12):
        temp_input = Lambda(lambda x: x[:, :, lead:lead+1])(inputs)

        if params.get('is_regular_conv', False):
            layer = add_conv_layers(temp_input, **params)
        else:
            layer = add_resnet_layers(temp_input, **params)

        stack_layers.append(layer)
            
    merged = tf.keras.layers.concatenate(stack_layers, axis=2)

#     keras.layers.average(inputs)
    
#     output = add_output_layer(layer, **params)
    output = add_output_layer(merged, **params)
